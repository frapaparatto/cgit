...
My original `src/core/hash.c` used Apple's CommonCrypto (`CC_SHA1`), which
works but locks the project to macOS. I replaced it with OpenSSL's `SHA1()` from
...
- **Digest call** — `CC_SHA1(data, len, hash)` replaced by
  ...
  `SHA1()` returns a pointer (NULL on failure), so I check the return value
  for robustness.
...
- I added Homebrew OpenSSL detection on macOS (`brew --prefix openssl` →
  `OPENSSL_ROOT_DIR`) so `find_package(OpenSSL)` works out of the box.
- I enabled `CMAKE_EXPORT_COMPILE_COMMANDS` so clangd picks up the correct
