...
I analyzed `refactoring/sample_dispatch.c` — a simplified version of how git's
...
I created the `docs/` directory structure to track project knowledge in git:

- `docs/learning/` — learned patterns and concepts
- `docs/journal/` — daily logs (this file)
- `docs/decisions/` — implementation trade-off notes (to be filled)

Previously, my notes lived in `refactoring/` which is gitignored. The new
`docs/` structure is tracked so that project evolution is visible.
